...
@progress for env_type = 1:env_types
@progress for rep = 1:reps
    ...
    env_df = CSV.read("./data/landscape_data_v1/env_$rep.csv", DataFrame)
    # emigration probabilities
    disp_mat = CSV.read("./data/landscape_data_v1/disp_mat_$rep.csv", DataFrame)
    disp_mat = Matrix(disp_mat)
    disp_mat = disp_mat[:, 2:end]
    # coordinates of patches
    landscape = CSV.read("./data/landscape_data_v1/landscape_$rep.csv", DataFrame)
    ...
    for i = 1:maximum(env_df.time)
        ...
        env_mat2[i, :] = env_df[env_df.time.==i, env_type + 1]
    end
    ...
    σ_niche_V = exp.(range(log(0.0001), stop = log(100), length = 10))
    # σ_niche_V = sqrt.(range(0.001^2, stop = 10^2, length = S))
    # σ_niche = reshape(repeat(σ_niche_V, inner = [M]), M, S)
    # competition scenarios
    α_V = [0,0.5,1.5]
    # dispersal probability
    a_Vect = exp.(range(log(1e-5), stop = log(1), length = 10))
    ...
    @progress for k = 1:(length(α_V)+2)
        ...
        @progress for i = 1:length(a_Vect)
            ...
            @progress for j = 1:length(σ_niche_V)
                ...
                # sampling times (only sample 8 times)
                sampV = convert(Array{Int64}, collect((burn_in+800):200:Tmax))
    ...
    # for every rep
    CSV.write("C:/Users/andy/Downloads/plthompson-Meta_com_framework-36ca456_ver1/outputs/outputfile_$rep"*"_$env_type.csv", Model_df)
end
end
