...
using FLoops # parallel calculation
...
σ_niche_V = exp.(range(log(0.001), stop = log(10000), length = 10))
# competition scenarios
α_V = [0,0.5,1.5]
# dispersal: number of emigrants
a_Vect = 1:5
# dispersal: ability to disperse far
b_V = exp.(range(log(0.01), stop = log(100), length = 10))
# intrinsic growth
grow_V = 1:3:10
...
for rep = 1:reps
    ...
    for (env_type, k, i, j, l, m) in collect(Iterators.product(1:2, 1:(length(α_V)+2), 1:length(a_Vect), 1:length(σ_niche_V), 1:length(grow_V), 1:length(b_V)))

            print(("env_type = $env_type", "k = $k", "i = $i", "j = $j", "l = $l", "m = $m"))
            # environment matrix: col = patches, row = time, value = env1 or env2
            env_mat2 = zeros(maximum(env_df.time), M)
            if (env_type == 1)
                for ii = 1:maximum(env_df.time)
                    env_mat2[ii, :] = env_df.env1[env_df.time.==ii]
                end
            else
                for ii = 1:maximum(env_df.time)
                    env_mat2[ii, :] = env_df.env2[env_df.time.==ii]
                end
            end
