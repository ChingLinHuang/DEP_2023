...
data.median<- all.results.df %>%
  ...
  ungroup() #%>% 
# filter(reps > 10) # Don't filter data in this case

data.median <- data.median %>% 
  mutate(competition = recode(alpha, no = "no",
                              equal = "equal",
                              stable = "stabilizing",
                              mixed = "mixed competition",
                              patch = "CC"))

data.median$competition <- factor(data.median$competition, levels = c("no", "equal", "stabilizing", "mixed competition", "CC"), ordered = TRUE)
...
#Figure 3####
hold <- all.results.df %>%
  filter(sig_niche > 90 | sig_niche < 0.02) %>%
  #filter(dispersal < max(dispersal)) %>% 
  mutate(competition = recode(alpha, no = "no",
                              stable = "stabilizing",
                              mixed = "mixed",
                              equal = "equal",
                              patch = "CC")) %>%
  mutate(competition = factor(competition, levels = c("no", "equal", "stabilizing", "mixed", "CC"), ordered = TRUE)) %>% 
  ...
  mutate(reps = length(unique(rep)))  %>% 
  # filter(reps > 10) %>% # Don't filter in this case
...
hold$sig_niche_names <- "narrow abiotic niche (\u03C3 = 0.01)"
hold$sig_niche_names[hold$sig_niche > 90] <- "wide abiotic niche (\u03C3 = 100)"
