...
@progress for rep = 1:reps
    ...
    σ_niche_V = exp.(range(log(0.0001), stop = log(100), length = 30))
    # σ_niche_V = sqrt.(range(0.001^2, stop = 10^2, length = S))
    # σ_niche = reshape(repeat(σ_niche_V, inner = [M]), M, S)
    # competition scenarios
    α_V = [0,0.5,1.5]
    # dispersal probability
    a_Vect = exp.(range(log(1e-5), stop = log(10), length = 30))
    ...
    # for every rep
    CSV.write("C:/Users/andy/Downloads/Thompson_etal2020_outputs_20210728/outputfile_$rep.csv", Model_df)
end
