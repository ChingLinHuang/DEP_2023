######
## 2021/10/15
##  Since I ignored some variables in the simulated data, there may be some variables that appear in the original code. I removed them.
##  
######
...
for(i in 1:rep){
  print(i)
  load(paste("./outputs/rdata/outputfile_", i, "_", 1, ".RData", sep = ""))
  ...
  all_model_summary <- model_summary %>%
    left_join(model_summary_gamma) %>%
    left_join(model_summary_t_gamma)
