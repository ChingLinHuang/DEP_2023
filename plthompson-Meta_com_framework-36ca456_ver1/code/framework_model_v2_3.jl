...
for rep = 1:reps
    @time for env_type = 1:2
        ...
        σ_niche_V = exp.(range(log(0.001), stop = log(1000), length = 8))
        # competition scenarios
        α_V = [0,0.5,1.5]
        # dispersal: number of emigrants
        a_Vect = 1:2:11 # control the expected number of emigrants
                    ...
                    for l = 1:length(grow_V)
                        ...
                        for m = 1:length(b_V)
                            ...
                            for gen = 1:Tmax
                                ...
                                if k < (length(α_V) + 2)
                                    disp_mat = disp_mat.^b * Diagonal(1 ./ vec(sum(disp_mat.^b, dims = 2)))
                                    emigrants = [rand(Binomial(n, 0.05)) for n in N] * a
                            ...
                            CSV.write("./outputs/outputfile_v3/outputfile_rep$rep" * "env$env_type" * "k$k" * "i$i" * "j$j" * "l$l" * "m$m" * ".csv", Model_df_1)
                        end
                    end
                end
            end
