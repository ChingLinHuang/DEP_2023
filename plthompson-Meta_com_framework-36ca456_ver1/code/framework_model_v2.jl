...
@progress for rep = 1:reps
    @progress for env_type = 1:2
        ...
        σ_niche_V = exp.(range(log(0.001), stop = log(10000), length = 20))
        ...
        b_V = exp.(range(log(0.01), stop = log(100), length = 20))
        # intrinsic growth
        grow_V = 1:3:10
        ...
        @progress for k = 1:(length(α_V)+2)
            ...
            @progress for i = 1:length(a_Vect)
                ...
                @progress for j = 1:length(σ_niche_V)
                    σ_niche = σ_niche_V[j]
                    @progress for l = 1:length(grow_V)
                        grow = grow_V[l]
                        @progress for m = 1:length(b_V)
                            b = b_V[m]
                            global Model_df
        ...
        # for every rep
        CSV.write("C:/Users/andy/Downloads/plthompson-Meta_com_framework-36ca456_ver1/outputs/outputfile_v2/outputfile_$rep.csv", Model_df)
    end
end
# 20210728
